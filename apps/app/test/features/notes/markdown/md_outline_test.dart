...
import 'package:alliswell/src/features/notes/markdown/md_outline.dart';
...
void main() {
  group('slugs (D16) — folded, not truncated', () {
    test('Turkish letters FOLD instead of vanishing', () {
      ...
      expect(markdownSlug('Türkçe Başlık'), 'turkce-baslik');
      expect(markdownSlug('Işık ve Iğdır'), 'isik-ve-igdir');
      expect(markdownSlug('Çay İç'), 'cay-ic');
    });

    test('GitHub rules do the rest', () {
      expect(markdownSlug('Hello, World!'), 'hello-world');
      expect(markdownSlug('  spaced   out  '), 'spaced-out');
      expect(markdownSlug('C++ & Rust'), 'c-rust');
      expect(markdownSlug('Kurulum (2026)'), 'kurulum-2026');
    });

    test('duplicate headings get GitHub-style suffixes', () {
      final doc = parseMarkdown('## Kurulum\n\n## Kurulum\n\n## Kurulum\n');
      final headings = outlineHeadings(doc);
  ...
  group('anchors resolve (D16)', () {
    test('#türkçe-başlık finds its heading', () {
      ...
      final hit = headingForAnchor(headings, 'türkçe-başlık');
    ...
    test('the already-folded form works too, and so does the raw heading', () {
      ...
      expect(headingForAnchor(headings, 'turkce-baslik'), isNotNull);
      expect(headingForAnchor(headings, 'TÜRKÇE BAŞLIK'), isNotNull);
    });
    ...
    test('a percent-encoded anchor is decoded first', () {
      ...
      expect(
        headingForAnchor(headings, Uri.encodeComponent('türkçe-başlık')),
        isNotNull,
      );
    });
    ...
    test('a stray % does not throw the jump away', () {
      ...
      expect(headingForAnchor(headings, 'yüzde-%100-bölüm'), isNotNull);
    });

    test('an anchor nobody wrote returns null, it does not guess', () {
      final doc = parseMarkdown('## Giriş\n');
      expect(headingForAnchor(outlineHeadings(doc), 'yok-boyle'), isNull);
    });
  });
  ...
  group('the tree (D13)', () {
    test('nests by level', () {
      ...
      final tree = buildOutline(outlineHeadings(doc));
