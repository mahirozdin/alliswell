...
its tools are an allowlist of **24** (ADR-0022 amendment): readers (`search`, `list_*`, `get_*`),
the task write wave (`create_task`, `update_task`, `complete_task`, `reopen_task`, `snooze_task`,
the checklist pair, `acknowledge_reminder`) and the note/project/tag writes (`create_note`,
`update_note`, `link_note`, `unlink_note`, `create_project`, `update_project`, `create_tag`) —
with **no delete**, and workspace membership is re-checked per request. Writes never touch SQL
directly: they call the same domain layer REST does (`db/tasks.js`, `db/notes.js`,
`db/projects.js`, `db/tags.js`, `db/reminders.js` — ADR-0022 §4), and the audit + idempotency
bookkeeping every one of them owes lives in one place (`lib/mcp/actions.js`, OPH-262). Domain
refusals surface as tool results carrying their stable code; 5xx stays opaque. `list_files` is
metadata only — bytes and presigned URLs never cross this boundary. Track B is **in-app BYOK**:
...
- **Replica migrations:** bump `schemaVersion` in `sync/db/database.dart` AND add the
  ...
  A step that ALTERs a table a device may already hold needs a fixture that starts where
  ...
  Prove the step by deleting it and watching that fixture go red (OPH-344).
