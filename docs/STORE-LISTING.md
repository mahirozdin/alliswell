...
• Home-screen widgets mirroring your Home buckets (iPhone, iOS 26 and later).
...
• Ana ekran widget'ları, Ana ekrandaki gruplarınızı yansıtır (iPhone, iOS 26 ve üzeri).
...
| **"Widgets on iPhone, Android and Mac"**                    | There is **no macOS widget target**. Also, the iOS widget extension's deployment target is **26.2** (`IPHONEOS_DEPLOYMENT_TARGET` in the widget build configs) while the app itself targets iOS 14 — so the widget only appears on iOS 26.2+. That is why the Apple copy says _"iPhone, iOS 26 and later"_. Lower the extension's deployment target if you want the claim to be unqualified.                                                                                                                                                                                                                               |
