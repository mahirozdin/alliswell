...
**This table is generated** (OPH-317). It is rendered from
`apps/app/lib/src/notifications/platform_matrix.dart` — the same declaration
`AwPushMessaging` obeys before it asks the plugin for a token — and
`npm run check:notify-matrix` fails the build when the two disagree. §3 once
described a web permission flow that no line of code performed, for months;
that was issue #16, and this is what stops the next one.
...
The prose below says what a column cannot:
