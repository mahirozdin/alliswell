...
| ➡️ **Next task**         | **OPH-180 — uygulama içi alarm sesi** (`AlarmFeedback`'in ses yatağı; round 9 #6/#8). **YENİ BAĞIMLILIK KATEGORİSİ** → [ADR-0015](adr/0015-alarm-delivery-and-reminder-profiles.md) §7 zaten kabul: `just_audio` + `audio_session` (alternatif `audioplayers`) — seçim ilk implementasyon turunda şu gereksinimlere göre: döngü, iOS `AVAudioSession` **`.playback`** (uygulama ÖNDEyken sessiz anahtarını meşru şekilde aşar; arka plan audio hilesi DEĞİL), Android `USAGE_ALARM`, masaüstü+web, testlerde fakelenebilir. Mevcut seam korunacak: `AudioAlarmFeedback implements AlarmFeedback` (testler `SilentAlarmFeedback`'te kalır — bekleyen timer/kanal yok). Web'de autoplay engellenirse dürüst düşüş (görsel+titreşim + "sesi başlat" düğmesi). **Bitti:** OPH-171…179.
