...
| ➡️ **Next task** | **Epic 14'ün kod+doküman işi BİTTİ** (OPH-150…156 ✅, 157 kod+docs ✅). Kalan tek şey **OPH-157'nin manuel matrisi** (gerçek R2 + web CORS turu + cihazlar; STATE'e matris) — Epic 12/13 cihaz turuyla TEK fiziksel oturumda koşulabilir. Cihazsız sıradaki aday: OPH-143'ün Dart overlay kısmı (Epic 13). | Veri katmanı (153) hazır: `targetFilesProvider`/`projectFilesProvider`/`uploadsProvider`/`fileUrlCacheProvider` UI'ın bekleyeceği her şeyi veriyor. Cihaz turu geldiğinde Epic 12/13 kuyruğu (OPH-140 matrisi + OPH-131 Xcode) araya alınabilir. **Yerel not:** bu makinede MinIO **9010**'da (9000 = kullanıcının ssh tüneli; `.env`'de `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010` — 3306→3307 durumunun aynısı). |
