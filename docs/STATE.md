...
| ➡️ **Next task**         | **OPH-177 — erteleme netliği** (round 9 #6.5/#6.6). Özet: erteleme seçenekleri **ne olacağını yazar** ("5 dk sonra tekrar çalar") + erteledikten sonra snackbar ("22:52'de tekrar çalacak"); görev satırında/detayda **"Ertelendi — 22:52"** göstergesi (bugün `snoozedUntil` hiçbir yerde görünmüyor); **`reminders.snooze_count`** migration + REST/sync artırımı → planner'daki `notif.afterSnooze` **sayılı** varyanta geçer (`{count}. tur`; metin 176'da sayısız olarak kondu, tek satır değişecek); **özel ertele** (tarih-saat seçici; `POST /tasks/:id/snooze { snoozeUntil }` zaten var). **Bitti:** OPH-171…176. 176'dan taşınan: **iOS ses çözümleme bekçisi OPH-181'de** (native kanal orada kurulacak).
