...
| ➡️ **Next task** | **EPIC 15 KOD TAMAM (OPH-160…170 hepsi ✅).** Kalan: kapanış turu — web rebuild + tarayıcı doğrulaması, README/ROADMAP dokunuşları (arama+pano+Dosyalar satırları), gerçek Google hesabıyla OPH-160 canlı teyidi (kullanıcı), sonra v0.4.0 etiketi. Cihaz kuyruğu (OPH-131…157 manuel matris) hâlâ kullanıcı turunu bekliyor. | **Yerel not:** MinIO **9010** (`.env` `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010`; 3306→3307 durumunun aynısı). **Alarm ring'i denemek için:** acil bir görevi geçmiş bir due saatiyle işaretle → uygulama açıkken ring ekranı çıkar (autoShow prod'da açık). **Demo hesap (yerel):** demo@alliswell.test / demo1234. |
