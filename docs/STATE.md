...
| ➡️ **Next task**         | **OPH-178 — süresiz erteleme / "alarmı sustur"** (round 9 #6.7). Özet: `tasks.alarms_muted_at` TIMESTAMP NULL migration + sync FIELD (`alarmsMutedAt`, isoOrNull) + REST PATCH alanı (**yeni endpoint YOK**); motor tarafı **HAZIR** — `alarmInstantsFor` zaten `task.alarms_muted_at != null` ise boş liste döndürüyor (OPH-175'te ileriye dönük yazıldı), yani kolon gelince sunucu kendiliğinden doğru davranır; app: drift v11 + `taskAlarmInstants`'a aynı kontrol + ring ekranında "Süresiz ertele", bildirim aksiyonu `mute`, detayda switch, görev satırında `notifications_off` çipi + "Geri aç"; **görev AÇIK kalır** (susturmak tamamlamak değildir — DESIGN §11 A5). Geri açınca an geçmişse dürüstçe söyle. **Bitti:** OPH-171…177.
