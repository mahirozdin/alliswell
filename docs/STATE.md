...
| ➡️ **Next task**         | **OPH-181 — zil sesi kütüphanesi + özel ses yükleme** (round 9 #7 sesler). Özet: paketli sesler (`aw_alarm` + 2 kısa hatırlatıcı tonu — iOS `.caf` ≤30 sn + Android `res/raw` + **Dart asset**, üç yüzey aynı dosyayı duysun); özel yükleme mevcut R2 boru hattıyla (`targetType:'workspace'` + ayrılmış "Zil sesleri" klasörü), seçim cihaz-yerel (`alliswell_alarm_sound`/`alliswell_reminder_sound` = `bundled:…` | `file:<id>`); **cihaza kurulum:** iOS presigned GET → `<container>/Library/Sounds/<hash>.caf`, Android **ses başına kanal** (`urgent_alarms_v3_<hash>`) + bayat kanal temizliği; **format dürüstlüğü** (≤30 sn + caf/wav/aiff; mp3 iOS bildiriminde çalışmaz → yükleme anında söyle, "yalnız uygulama içi ses" alternatifi); **OPH-176'dan taşınan iOS ses çözümleme bekçisi** burada (çözülemezse günlüğe `degraded` + Ayarlar'da söyle); Ayarlar → Hatırlatıcı Sistemi'ne "Alarm sesi"/"Hatırlatıcı sesi" satırları + **önizleme** (OPH-180'in çaları hazır). **Bitti:** OPH-171…180.
