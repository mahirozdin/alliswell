...
| ➡️ **Next task** | **OPH-153** (app: drift v5 `files` replikası + FileStore + UploadService) → 154 task detayı → 155 proje Files sekmesi → 156 not embed'leri. **API dikeyi (150-152) TAMAM** — app'in ihtiyacı olan her uç mevcut. Cihaz turu geldiğinde Epic 12/13 kuyruğu (OPH-140 matrisi + OPH-131 Xcode) araya alınabilir. **Yerel not:** bu makinede MinIO **9010**'da (9000 = kullanıcının ssh tüneli; `.env`'de `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010` — 3306→3307 durumunun aynısı). |
