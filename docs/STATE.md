...
| ➡️ **Next task**         | **OPH-179 — hatırlatıcı profili: kaç tane, kaç dakikada bir** (round 9 #7) + Ayarlar'da **"Hatırlatıcı Sistemi Ayarları"** ekranı. Özet: saf `ReminderProfile` (`notifications/reminder_profile.dart`; artan sıra, tekilleştirme, **ardışık ≥1 dk**, ilk ≥0, **≤20 slot**, bozuk → fabrika `[0,2,5,10,30]`), `kUrgentChainOffsets` sabiti KALKAR → `planNotifications(..., profile:)`; cihaz-yerel `alliswell_reminder_profile`; **64 slot gerçeği ekranda** ("bu profille ~N alarm tam kapsanır", >10 slotta uyarı); editör: hazır profiller (Sakin 1 / Standart 5 / Israrcı 10) + adım listesi (dakika stepper, araya ekle, sil) + **canlı zaman çizelgesi** (kullanıcının tarih biçimiyle) + fabrikaya dön; **sürükle-bırak yalnız erteleme presetlerinin sırasında** (sıralı zincirde anlamsız — DESIGN §18 N4). **Bitti:** OPH-171…178.
