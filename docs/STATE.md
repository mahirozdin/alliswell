...
| ➡️ **Next task** | **OPH-167 — arama (TR-fold motor + Home/Notlar/Projeler).** Sonra sırayla: 168 (pano), 169 (klasör API), 170 (Dosyalar UI). | **Yerel not:** MinIO **9010** (`.env` `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010`; 3306→3307 durumunun aynısı). **Alarm ring'i denemek için:** acil bir görevi geçmiş bir due saatiyle işaretle → uygulama açıkken ring ekranı çıkar (autoShow prod'da açık). **Demo hesap (yerel):** demo@alliswell.test / demo1234. |
