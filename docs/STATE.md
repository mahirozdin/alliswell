...
| ➡️ **Next task**         | **OPH-175 — "görev saati de bir alarmdır": iki alarm örneği** (API+app). **Başlamadan oku:** [ADR-0015](adr/0015-alarm-delivery-and-reminder-profiles.md) + NOTIFICATIONS §5(a). Özet: `effectiveRemindAt` → `alarmInstantsFor(task)`; `reminders.kind` ENUM('remind','due') migration (unique index YOK, "en yeni aktif satır" kalıbı korunur; backfill remind_at null + urgent+due → 'due'); `reconcileTaskReminder` türler üzerinde döner; sync/REST `kind` yayar; `POST /tasks/:id/snooze` opsiyonel `reminderId`; app drift **v8** (`from >= 7` guard'ı) + `watchAlarms` iki sentetik tür (`local:remind:` / `local:due:`) + `acknowledge` türü ayrıştırır; yeni gövde `notif.dueNow`. **UI paketi ✅:** OPH-171 (yenileme) · 172 (Home tek kaydırma) · 173 (hiza + yarın) · 174 (tarih biçimi) — hepsi 2026-07-28.
