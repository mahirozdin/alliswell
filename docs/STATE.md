...
| ➡️ **Next task** | **OPH-157** — Epic 14 hardening/QA: workspace kullanım göstergesi (usage ucu UI'ı), web CORS gerçeklik turu + büyük dosya matrisi (cihaz/web — Epic 12/13 cihaz turuna binebilir), README 'Attachments' kurulum bölümü + ATTACHMENTS.md'nin gerçeğe karşı doğrulanması, SECURITY/ROADMAP güncellemeleri. Veri katmanı (153) hazır: `targetFilesProvider`/`projectFilesProvider`/`uploadsProvider`/`fileUrlCacheProvider` UI'ın bekleyeceği her şeyi veriyor. Cihaz turu geldiğinde Epic 12/13 kuyruğu (OPH-140 matrisi + OPH-131 Xcode) araya alınabilir. **Yerel not:** bu makinede MinIO **9010**'da (9000 = kullanıcının ssh tüneli; `.env`'de `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010` — 3306→3307 durumunun aynısı). |
