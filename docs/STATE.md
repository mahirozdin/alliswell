...
| ➡️ **Next task** | **OPH-168 — Pano (Kanban) Home görünümü.** Sonra: 169 (klasör API), 170 (Dosyalar UI). | **Yerel not:** MinIO **9010** (`.env` `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010`; 3306→3307 durumunun aynısı). **Alarm ring'i denemek için:** acil bir görevi geçmiş bir due saatiyle işaretle → uygulama açıkken ring ekranı çıkar (autoShow prod'da açık). **Demo hesap (yerel):** demo@alliswell.test / demo1234. |
