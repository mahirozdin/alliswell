...
**Last updated:** 2026-07-19 (**OPH-143 → foreground alarm ekranı + dürüst uyarı banner'ları —
Epic 13'ün son CİHAZSIZ işi, BİTTİ:** urgent alarm due olunca uygulama açıksa tüm ekranı kaplayan
`AlarmRingScreen` (Onayla + snooze + Tamamla/Aç, `PopScope`; masaüstü/web'in TEK alarm yüzeyi,
mobilde OS bildiriminin foreground eşi), `AlarmOverlayController` (replika alarm feed + foreground
timer wheel) HomeShell'de tur'un ÜSTÜNDE; ses enjekte edilebilir seam (`AlarmFeedback` — haptik
bugün, audio cihaz turuna); Home'da `AlarmDegradationBanner` (bildirim kapalı / Android exact-alarm
reddi, worst-first, tap izin akışı). Ring kararı saf fn, auto-show testlerde OFF (OPH-111 idiomu).
Yeni `alarmSupportProvider` + `alarm.*` i18n (en+tr), DESIGN §11, NOTIFICATIONS §3. **App 322/322,
analyze + check:i18n + kontrast (FAILURES: 0) temiz.** Bununla birlikte projenin CİHAZSIZ backlog'u
tükendi — kalan her iş cihaz/Xcode/kullanıcı turu. — Önceki: **Design round 8 → "Liquid Glass v2" görsel yenileme, ADR-0012:**
...
| ➡️ **Next task** | **CİHAZSIZ KALAN İŞ KALMADI.** OPH-143 (Epic 13'ün son cihazsız işi) bitti; Epic 14 kod+docs bitti. Sıradaki her şey **fiziksel cihaz / Xcode / kullanıcı aksiyonu** bekliyor: **OPH-140** alarm matrisi (iPhone Sleep Focus + Android ALARM stream), **OPH-141** AlarmKit köprüsü (iOS 26 native + cihaz), **OPH-142** critical-alerts başvurusu (Mahir, Account Holder), **OPH-131…136** widget Xcode target + cihaz turu, **OPH-157** gerçek R2 + web CORS + cihaz foto matrisi. Hepsi TEK fiziksel oturumda koşulabilir. | **Yerel not:** MinIO **9010** (`.env` `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010`; 3306→3307 durumunun aynısı). **Alarm ring'i denemek için:** acil bir görevi geçmiş bir due saatiyle işaretle → uygulama açıkken ring ekranı çıkar (autoShow prod'da açık). |
