...
| ➡️ **Next task** | **OPH-170 — global "Dosyalar" bölümü (app):** drift v7 (folders + fileRows.folderId — v6 arama koydu, bu v7 olacak), applier `folder`, FolderStore, nav `AppSection.files`, klasör gezgini + Kaynaklar. Sonra epic kapanışı (web rebuild + doğrulama + README/ROADMAP). | **Yerel not:** MinIO **9010** (`.env` `MINIO_PORT=9010` + `STORAGE_S3_ENDPOINT=http://127.0.0.1:9010`; 3306→3307 durumunun aynısı). **Alarm ring'i denemek için:** acil bir görevi geçmiş bir due saatiyle işaretle → uygulama açıkken ring ekranı çıkar (autoShow prod'da açık). **Demo hesap (yerel):** demo@alliswell.test / demo1234. |
