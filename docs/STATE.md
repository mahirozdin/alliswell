...
| ➡️ **Next task**         | **OPH-174 — tarih/saat gösterimi: tek kaynak + kullanıcı ayarı** (`core/date_format.dart` **zaten var** — OPH-173 `awInitialPickerDate`'i oraya koydu; şimdi biçimlendiriciler + presetler eklenecek: `system` varsayılan, tr'de 31.12.2026 23:59). Bağlanacak **9 çağrı yeri** TASKS'ta satır satır yazılı; `dateFormatProvider` `core/persisted_prefs.dart`'a (diğer PersistedChoice'ların yanına), Ayarlar satırı **sonucu gösterir, kalıbı değil** (DESIGN §17 D2), seçim **widget snapshot'ına da** taşınmalı. `external_event_tile` düz `StatelessWidget` → `ConsumerWidget`'a çevrilecek (tek yer). **OPH-171/172/173 ✅ (2026-07-28).** Sonra: **175'ten itibaren alarm belkemiği** (migration + API; ADR-0015 okunmadan başlanmaz).
