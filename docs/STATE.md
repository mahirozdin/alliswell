...
| ➡️ **Next task**         | **OPH-182 — iOS 26 AlarmKit'i GERÇEKTEN devreye al** — **bu bir CİHAZ işi** (round 9 #8'in çözümü). Kod tarafı hazır: Dart lane + `AlarmKitBridge.swift` yazılı, `NSAlarmKitUsageDescription` var, sesler `Library/Sounds`'a kuruluyor (OPH-181) → `AlertConfiguration.AlertSound.named` için hazır. Yapılacaklar (Xcode, iOS 26 makinesi): (1) `AlarmKitBridge.swift`'i **Runner hedefine** ekle + `AppDelegate`'te kur ([ALARMKIT_SETUP.md](../apps/app/ios/Runner/ALARMKIT_SETUP.md) adım 2-3); (2) **round 9 araştırmasının yeni bulgusu** — `AlarmAttributes<AWAlarmMetadata>` okuyan **widget extension `ActivityConfiguration`** + iki Info.plist'te `NSSupportsLiveActivities` (yoksa sistem alarmları düşürebiliyor); (3) buton metinlerini kanaldan yerelleştir, `sound:` parametresine kurulu sesi ver, `.countdown`'ı profilin ilk erteleme değerine bağla; (4) SDK tip adlarını doğrula; (5) **cihaz DoD matrisi:** sessiz anahtarı AÇIK + Uyku Odak AÇIK + ekran KİLİTLİ → tam ekran çalıyor, Onayla senkronize, Ertele yeniden çalıyor, iOS<26 bildirim zincirine düşüyor, **alarm günlüğü lane'i doğru yazıyor**. Sonra **OPH-183** (Apple Watch ölçümü). **Bitti:** OPH-171…181 (kodsal her şey).
