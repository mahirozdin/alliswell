...
| Push messaging is optional too (OPH-319) | `firebase_messaging` is a dependency, but `AwPushMessaging` asks it for nothing unless `AwFirebase.isConfigured` — so a clone with no config file registers no token, the server has nothing to send to, and the app behaves exactly as it did before Epic 30. It is **never** used on the web, which is reached with VAPID (ADR-0038 §6) and so keeps "web has no implicit config" true. iOS additionally needs an **APNs key uploaded to the Firebase project** and the `aps-environment` entitlement that ships in `Runner.entitlements`; without the key, a token is issued and nothing is ever delivered — a silence no test can catch. |
| The Performance plugin trails AGP         | `com.google.firebase.firebase-perf` must be **≥ 2.0.2** on AGP 9. The 2.0.1 jar still references `com.android.build.api.transform.Transform`, the bytecode-transform API removed in AGP 8.0, so Gradle cannot instantiate the plugin at all: `bundleRelease` dies with "Could not generate a decorated class for type FirebasePerfPlugin". The message names Gradle and the cause is a missing class — check the jar (`unzip -p perf-plugin-<v>.jar '*.class' \| strings \| grep transform/Transform`) before believing a version is new enough. |
