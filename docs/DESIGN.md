...
| L5 | **Deliberately not sorted:** Home (its grouping *is* its order), Projects (manual `sortOrder` is the product), board columns. **Starred notes ARE grouped above the list, always, before any sort applies** — corrected 2026-08-16 on the owner's call; this rule originally said the opposite. | Written non-goals keep the next round from reading absence as neglect. On the star, the original reasoning was wrong about what the gesture means: pinning is how someone says *keep this where I can see it*, so an order that can bury a starred note turns the star into a decoration. Inside each group the chosen order applies unchanged, so nothing else about §34 moves. |
