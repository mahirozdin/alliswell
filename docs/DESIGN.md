...
| R3 | **Hex never appears.** No hex input, no hex label, nowhere. The quill stock dialog's `showColorButton`/`showBackgroundColorButton` are disabled and replaced by our own toolbar buttons opening `AwColorPicker`. | Round-1 rule; the stock dialog is the measured violation this section exists to kill. **Status (OPH-259):** the four AllisWell-owned pickers are converted; the stock quill dialog is still reachable in Live mode and stays until its replacement exists, because removing the buttons first would delete the capability rather than fix it (§22 in reverse). It is blocked on R4's correction below — not forgotten. |
| R4 | ~~**The editor's palettes are curated and theme-checked.** Text colors: 12 hues legible on both themes' surfaces; highlight colors: 6 fills that keep body text ≥ 4.5:1 in both themes.~~ **CORRECTED (OPH-259, measured 2026-08-15): as written this rule cannot be satisfied, because a Quill `color`/`background` attribute stores one fixed hex and the surface (or ink) under it changes with the theme.** Measured: of 18 candidate text hues, **zero** clear 4.5:1 against BOTH `#FFFFFF` (light surface) and `#151F3C` (dark surface) — they run 5–8 in light and 2–3 in dark. Highlights are worse: a fill has to stay readable under `#0F1B2E` ink in light and `#EAF0FD` ink in dark, and since those two ratios multiply to at most 21, both can only reach ~4.6 at one exact mid lightness — the best candidate, mid-grey `#808080`, measures **4.37 / 3.46** and fails both while looking like nothing anyone would call a highlight. **The colour therefore cannot be stored as a raw hex and still keep this promise.** Two honest ways forward, to be decided before any editor colour ships: **(a)** store a semantic id and resolve it to a per-theme hex when rendering — which is exactly what this app's own `==mark==` already does (`aw_markdown.dart` tints `tokens.warning` at 28 % alpha, so the body ink never changes); or **(b)** highlight sets fill **and** ink together as one fixed pair, the way a physical highlighter and Word both behave, making it theme-independent by construction. (a) keeps documents portable inside AllisWell and breaks nothing for other renderers that ignore an unknown attribute; (b) is simpler but writes a colour the user did not choose. | User-picked colour is data (G6 exception) — but a text colour that disappears in dark mode is a broken feature, not a preference. The original rule promised a gate that arithmetic cannot pass; saying so here is cheaper than a palette that looks verified and is not. |
