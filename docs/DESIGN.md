...
| D20 | **Paste is smart and reversible.** HTML pastes as markdown; a URL over a selection makes a link; a clipboard image uploads as an attachment and lands as an embed. One undo restores the raw paste. | "Smart" paste without one-step undo is hostile. *Completed in OPH-274:* the HTML half had SHIPPED and never once run — `Clipboard.getData('text/html')` returns null on every platform, because Flutter's clipboard channel implements `text/plain` and nothing else. Both flavours now come through the host (`markdown_forge`'s clipboard seam → `alliswell_docref`, the plugin ADR-0030 already built). A failed image upload falls back to the clipboard's text rather than pasting nothing, and the whole result is ONE controller assignment, which is what keeps it one undo. |
...
| D25 | **A document leaves room after its last line, in BOTH modes** (round 19c, OPH-282). Two numbers in one: the host's floating chrome (`MediaQuery.padding.bottom` — the shell's glass bar sits over the body, so without this the last paragraphs are not awkward but *unreachable*), plus a share of the viewport to scroll PAST the end. A share rather than a constant, so it means the same thing on a phone and a desktop and shrinks on its own when a keyboard takes half the screen. In the editor that space is a **tap target that puts the caret at the end** — blank space that places no caret is §22's dead affordance, and "tap below the text to keep writing" is the gesture every note app trains people to expect. | `editor.scrollBeyondLastLine` (VS Code, on by default); typewriter scrolling (iA Writer, Ulysses) is the same idea at its limit. The room must be scrollable CONTENT: a fixed inset on an `expands: true` field carves a permanent strip out of the visible area and still pins the last line to the bottom of a shorter box. |
...
| A5 | **Image attachments show as thumbnails**, other files as rows. | A grid of file names is not how anyone reviews photos. **Already true** (`FileLeadingThumb`) — the owner never saw it because they could not attach a photo in the first place. **Amended (OPH-245, owner decision):** the grid is **not** built; the 40 px leading thumbnail rows stay. A grid is a second row anatomy for the same data, and §10 F1 says the three homes render the SAME row — breaking F1 in one of them to solve a problem the viewer already solves is a bad trade. Reviewing photos happens *in the viewer*, by swiping. Only the tap wiring changed. |
| A6 | **Tapping an image opens a real viewer: pinch-zoom, double-tap zoom, pan, swipe between the images of that task, and the file name in the bar.** | The round's literal ask. **Amended (OPH-245, owner decision): no share and no save-to-gallery.** The viewer carries exactly the two actions the Files viewer already had — Open/Download and Delete. Two reasons: a gallery saver needs a new package and reopens the Android media-permission surface that [ADR-0027](adr/0027-attachment-capture-image-picker.md) closed **by measurement** (18 permissions in the release APK's binary manifest, none of them media or camera, enforced by `scripts/android/assert-permissions.sh`); and "Open / Download" already hands the bytes to the browser/OS, which is where saving belongs (ATTACHMENTS §2.2 — nothing is written to app storage in v1). **Delivered:** pinch (`maxScale: 6`), double-tap to 2.5× **about the tapped point** (`AwMotion.base`, `enter` in / `exit` out), pan, paging, filename + `n/m`, Escape and keyboard zoom/paging. The viewer renders in the dark theme whatever the app theme is, so its chrome stays contrast-safe on a black backdrop. |
| A7 | **One viewer, everywhere.** Task attachments, note embeds and the Files section all open the same component. | There were TWO — `showFileImageViewer` (files) and `_EmbedImageViewer`, private to `note_media.dart` and without even a loading or error builder — which is the §22 problem in its clearest form. **OPH-245 measured why they disagreed:** `FileRowTile` let a caller-supplied `onMore` swallow the tap for **every** kind, so an image opened the sheet in Dosyalar and the viewer in a project's Files tab. That was deliberate (OPH-170 had no other way to reach Move-to / Go-to-source), so the fix **added** an affordance rather than removing a branch: the tap now follows the file's kind, and the injected actions moved to the row's own ⋯ button — the one folder rows already use. |
| A11 | **The gallery is whatever the surface is showing, in the order it is showing it.** A task's attachments page in list order; the project Files tab pages in its **current sort**; a note pages in **document order**. | Swiping right has to land where the surface implied it would. A note's attachment list is `created_at DESC`, which has nothing to do with where the reader is looking — so the note gallery comes from the same `deltaToBlocks` walk the PDF exporter uses, and the two can never drift. Foreign `http(s)` embeds are not in the gallery, because they are not tappable in the body either. |
...
| A8 | **Failure is a stated reason, not a spinner.** Storage unconfigured, upload failed, file gone: each says which. | §10 F3, already the rule for attachments. **Measured (OPH-245): this was not deliverable.** `FileUrlCache.urlFor` caught `ApiException` and returned `null`, so "offline" and "the file is gone" arrived at the widget as the same nothing, and all of them rendered `file.couldNotOpen` ("Could not get a download link") — a sentence that is only true for one of them. The cache now keeps the code, and the viewer resolves six outcomes: offline · gone (404) · any other server code (through `localizedError`) · storage switched off · no link yet · the link worked but the bytes would not decode. `file.couldNotOpen` stays where it is still true — the two download paths, where you did ask for a link and there was none. |
...
| # | Rule | Why |
| --- | --- | --- |
| S1 | **The root Settings screen is an index, not a form.** It shows the account header, then one row per group — icon + title + a subtitle naming what lives inside ("Google Takvim, Apple Takvim, Yapay zeka") — then the sign-out card and the About row. Nothing else stays on the root. | An index the eye can scan in one pass; the subtitle is the map, so nobody has to open a group to learn what it holds. |
| S2 | **Seven groups, fixed:** Hesap (account header target, server URL, account deletion) · Genel (language, date format, default task time, **editor formatting** — round 19 —, quick-access bubble, app tour; **a Widget card — density and "Private widget" — OPH-337, only on platforms that have widgets**, a card of Genel rather than an eighth group) · Bildirimler & Alarmlar (notification privacy, alarm permission status, **test alarm** — round 19 —, reminder system, alarm log) · Entegrasyonlar (Google Calendar, Apple Calendar, AI — whose screen keeps the MCP connector card) · **Geliştirici (API erişimi, API dokümantasyonu — round 20)** · Veri (completed tasks, share log; import/export lands here when it ships) · Hakkında (version + licences, the existing About dialog). | Grouping is by what the user is trying to do, not by which subsystem implements it. Every existing row keeps its widget `Key` and i18n string — this is a re-homing, not a rewrite (§22: nothing becomes unreachable). **Amended by OPH-292 (was six groups, with API access inside Entegrasyonlar).** The original placement was right for what the API was in round 18 — one more thing the app could be wired to. It stopped being that when it grew a public reference, a Postman collection and a landing-page entry: an integration is something this app talks TO, and this is a surface other software is written AGAINST. The measured symptom was a report that the feature was missing in production when it had shipped two releases earlier — it was two levels down, behind a word ("Entegrasyonlar") that does not describe it. Putting webhooks or a second MCP surface behind the same door later is now additive rather than another re-home. |
...
| L5 | **Deliberately not sorted:** Home's GROUPING (its day headings *are* its order — since OPH-305 the order *inside* each group is sortable, the headings are not), Projects (manual `sortOrder` is the product), board columns. **Starred notes ARE grouped above the list, always, before any sort applies** — corrected 2026-08-16 on the owner's call; this rule originally said the opposite. | Written non-goals keep the next round from reading absence as neglect. On the star, the original reasoning was wrong about what the gesture means: pinning is how someone says *keep this where I can see it*, so an order that can bury a starred note turns the star into a decoration. Inside each group the chosen order applies unchanged, so nothing else about §34 moves. |
| L6 | **Task lists share one order and one preference** (OPH-338): the project's Tasks tab offers `kTaskSortChoices` through the same `AwSortMenuButton`, orders with Home's own comparator (`homeItemOrder`, lifted out of `groupTasksForHome` unchanged — done work still sinks, C1), and reads and writes `alliswell_tasks_sort`. The control closes the tab's quick-add row (L2: no new row). | L4's pattern — the project Files tab and the Files section share one choice — applied to tasks. Two task lists ordered two ways, or two definitions of "by priority", would each be something to explain. Until OPH-338 the tab was in creation order, which no one chose. |
